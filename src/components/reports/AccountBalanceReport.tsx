...
export default function AccountBalanceReport() {
  ...
  const getAccountStyle = (type: string) => {
    switch (type) {
      case 'SAVINGS': 
        return { icon: Wallet, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', bar: 'bg-emerald-500' }
      case 'SALARY': 
        return { icon: Briefcase, color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20', bar: 'bg-blue-500' }
      case 'CURRENT': 
        return { icon: CreditCard, color: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', bar: 'bg-indigo-500' }
      case 'CASH': 
        return { icon: Banknote, color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20', bar: 'bg-amber-500' }
      default: 
        return { icon: Landmark, color: 'text-slate-400', bg: 'bg-slate-500/10', border: 'border-slate-500/20', bar: 'bg-slate-500' }
    }
  }
  ...
  const getAccountTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'CURRENT': return 'Checking / Current'
  ...
  if (error) {
    return (
      <div className="p-6 bg-red-500/10 border border-red-500/20 rounded-2xl flex flex-col items-center text-center">
        ...
        <h3 className="text-lg font-semibold text-white">Report Failed</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-slate-950/50 border border-slate-800 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Total Net Worth</p>
        ...
        <div className="p-5 rounded-2xl bg-slate-950/50 border border-slate-800 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Active Accounts</p>
            <p className="text-2xl font-bold text-white mt-0.5">
        ...
        <div className="p-5 rounded-2xl bg-slate-950/50 border border-slate-800 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Asset Classes</p>
            <p className="text-2xl font-bold text-white mt-0.5">
                ...
                <div key={item.type}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-200">{getAccountTypeLabel(item.type)}</p>
                  ...
                  return (
                    <tr key={account.id} className="group hover:bg-slate-900/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-400">
                        {account.bank || <span className="text-slate-600 italic">N/A</span>}
                      </td>
                      <td className="px-6 py-4">
                        ...
                        {account.transactions && account.transactions.length > 0 ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs">
                              {new Date(account.transactions[0].date).toLocaleDateString()}
                            </span>
                          </div>
                        ) : (
