...
export default function SpendingTrendsReport({
    months = 6,
    onDataLoaded,
}: SpendingTrendsReportProps) {
                        ...
                        <div className="space-y-5">
                            {monthlyTrends.map((trend: any) => {
                                const maxAmount = Math.max(
                                    ...monthlyTrends.map((t: any) => t.total),
                                );
                                const percentage =
                                    maxAmount > 0
                                        ? (trend.total / maxAmount) * 100
                                        : 0;
                            ...
                            <tbody className="divide-y divide-slate-800">
                                {monthlyTrends.map(
                                    (trend: any, index: number) => {
                                        const prevTrend =
                                            monthlyTrends[index - 1];
                                        const change = prevTrend
                                            ? trend.total - prevTrend.total
                                            : 0;
                                        const changePercent =
                                            prevTrend && prevTrend.total > 0
                                                ? (change / prevTrend.total) *
                                                  100
                                                : 0;
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4">
                                                    {prevTrend ? (
                                                        <div
                                                            className={`flex items-center gap-1 ${change >= 0 ? "text-rose-400" : "text-emerald-400"}`}
                                                        >
                                                            {change >= 0 ? (
